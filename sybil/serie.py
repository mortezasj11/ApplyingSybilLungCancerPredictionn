...
class Serie:
    ...
    def _load_metadata(self, paths, voxel_spacing, file_type):
        ...
        """
        #vixel_spacing:  array([0.703125, 0.703125, 2.5     ])
        if file_type == "dicom":
            slice_positions = []
            processed_paths = []
            for path in paths:
            ...
            thickness = float(dcm.SliceThickness) #2.5
            pixel_spacing = list(map(float, dcm.PixelSpacing))#[0.703125, 0.703125]
            manufacturer = dcm.Manufacturer   #'GE MEDICAL SYSTEMS'
            voxel_spacing = torch.tensor(pixel_spacing + [thickness, 1])  #tensor([0.7031, 0.7031, 2.5000, 1.0000])
        elif file_type == "png":
